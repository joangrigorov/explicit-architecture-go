...
import (
	...
	"app/internal/infrastructure/persistence/ent/generated/blog/predicate"
	"context"
	"fmt"
	"math"
...
func (pq *PostQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("blog: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (pq *PostQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range pq.inters {
		if inter == nil {
			return fmt.Errorf("blog: uninitialized interceptor (forgotten import blog/runtime?)")
		}
	...
	for _, f := range pq.ctx.Fields {
		if !post.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("blog: invalid field %q for query", f)}
		}
	}
	if pq.path != nil {
