...
func (puo *PostUpdateOne) sqlSave(ctx context.Context) (_node *Post, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`blog: missing "Post.id" for update`)}
	}
	...
	if fields := puo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			if !post.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("blog: invalid field %q for query", f)}
			}
			if f != post.FieldID {
